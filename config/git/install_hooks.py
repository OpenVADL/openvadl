...
def find_git_root(from_path):
    try:
        ...
        print(
            "❌ Error finding git repository root. Are you inside a git repository?\n",
            e,
        )
        sys.exit(1)
...
def install_git_hook(hook_name, source_path, target_path):
    ...
    if sys.platform == "win32":
        ...
        if os.path.exists(target_path):
            ...
            print(
                f"❌  Existing hook conflicts with the intended installation.\n  Hint: Remove {target_path} and try again."
            )
            return
        else:
...
def check_python3_availability():
    try:
        ...
        args = (
            ["python", "--version"]
            if sys.platform == "win32"
            else ["/usr/bin/env", "python3", "--version"]
        )
