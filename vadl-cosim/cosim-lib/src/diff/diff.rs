...
use crate::{
    config::Config,
    diff::DiffEntry,
    ipc::cstructs::{MemAccessInfo, SHMRegister, MAX_CPU_COUNT, MAX_CPU_REGISTERS, SHMCPU},
};
...
pub fn diff_mem_access(mem_access_info1: &MemAccessInfo, mem_access_info2: &MemAccessInfo, config: &Config) -> Vec<DiffEntry> {
    let mut diffs = vec![];
    
    if mem_access_info1.size != mem_access_info2.size {
        diffs.push(DiffEntry::new(
            "mem.size",
            vec![mem_access_info1.size.to_string(), mem_access_info2.size.to_string()],
    ...
    if mem_access_info1.vaddr != mem_access_info2.vaddr {
        diffs.push(DiffEntry::new(
            "mem.vaddr",
            vec![mem_access_info1.vaddr.to_string(), mem_access_info2.vaddr.to_string()],
    ...
    if mem_access_info1.data_slice() != mem_access_info2.data_slice() {
        diffs.push(DiffEntry::new(
            "mem.data",
            vec![mem_access_info1.data_slice_fmt(), mem_access_info2.data_slice_fmt()],
