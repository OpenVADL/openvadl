...
/**
 * Tokenizer used by the language server. Provides LSP semantic tokens based on VADL Scanner tokens.
 *
 * <p>Must be in this package, otherwise it couldn't access {@link Token}.
 */
public class LspScannerTokenizer extends LspTokenizer {
   ...
   *                          client supports.
   */
  public LspScannerTokenizer(
      Map<String, Integer> tokenTypesMap, Map<String, Integer> tokenModifiersMap) {
  ...
  public List<Integer> getTokens(DocumentSnapshot snapshot) {
    Scanner scanner = new Scanner(
        new ByteArrayInputStream(snapshot.text().getBytes(StandardCharsets.UTF_8))
    );
  ...
  private int getTokenTypeFromScannerKind(int kind) {
    return getTokenTypeInteger(tokenKindsMap[kind]);
  }
}
