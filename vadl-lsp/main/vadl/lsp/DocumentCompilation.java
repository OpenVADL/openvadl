...
class DocumentCompilation {
  ...
  DocumentCompilation(Path path, VadlTextDocumentService documentService) {
    this.path = path;
  ...
  synchronized void clearCompilation() {
    if (compilationTask != null) {
      log.debug("{} compilation of {}", compilationTask.isDone() ? "Discard" : "Interrupt", path);
  ...
  private synchronized Future<CompilationInputAndResult> startCompilation()
    ...
    final var document = fileSystemSnapshot.getDocument(path);
    if (document == null) {
    ...
    compilationTask = documentService.server.executor.submit(() -> {
      var compilerResult = Frontend.compileToAstBestEffort(document.path,
          fileSystemSnapshot);
      ...
      log.debug("Compiled {} (version {})", path, document.version);
      return result;
    });
