...
createTargetCodeGenInfo(CodeGenModule &CGM)
{
    ...
    case llvm::Triple::[(${namespace})]:
        return create[(${namespace})]TargetCodeGenInfo(CGM);
    }
}
...
void CodeGenModule::Release()
{
    ...
    if (Arch == llvm::Triple::arm || Arch == llvm::Triple::armeb || Arch == llvm::Triple::thumb || Arch == llvm::Triple::thumbeb)
    {
...
void CodeGenModule::EmitVTablesOpportunistically()
{
    ...
    assert((OpportunisticVTables.empty() || shouldOpportunisticallyEmitVTables()) && "Only emit opportunistic vtables with optimizations");
...
void CodeGenModule::EmitGlobalAnnotations()
{
    ...
    llvm::Constant *Array = llvm::ConstantArray::get(llvm::ArrayType::get(
                                                         Annotations[0]->getType(), Annotations.size()),
                                                     Annotations);
...
llvm::Constant *CodeGenModule::EmitAnnotationLineNo(SourceLocation L)
{
    ...
    unsigned LineNo = PLoc.isValid() ? PLoc.getLine() : SM.getExpansionLineNumber(L);
                                   ...
                                   bool ExcludeDtor)
{
    ...
    {
        if (const CXXRecordDecl *Record = Context.getBaseElementType(Ty)->getAsCXXRecordDecl())
...
void CodeGenModule::EmitObjCPropertyImplementations(const ObjCImplementationDecl *D)
{
...
static QualType GeneralizeType(ASTContext &Ctx, QualType Ty)
{
    ...
    return Ctx.getPointerType(
        QualType(Ctx.VoidTy).withCVRQualifiers(Ty->getPointeeType().getCVRQualifiers()));
}
