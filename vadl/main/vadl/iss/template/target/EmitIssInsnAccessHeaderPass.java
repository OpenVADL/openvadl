...
public class EmitIssInsnAccessHeaderPass extends IssTemplateRenderingPass {
  ...
  public static final String ACCESS_FN_SIGNATURES_KEY = "insn_access_signatures";
  ...
  protected Map<String, Object> createVariables(PassResults passResults,
    ...
    final List<String> fnDefs = fieldAccesses.stream()
        .distinct()
        .map(AccessFunctionCodeGenerator::new)
        .map(AccessFunctionCodeGenerator::genFunctionSignature)
        .map(String::strip).map(s -> s + ";")
        .toList();

    variables.put(ACCESS_FN_SIGNATURES_KEY, fnDefs);
