...
public class ControlLogicPass extends Pass {
  ...
  public Object execute(PassResults passResults, Specification viam) throws IOException {
    ...
    for (Stage stage : mia.stages()) {
      enableMap.put(stage, new Signal(control.identifier.append(stage.simpleName() + "_en"),
          Type.bool()));
    }
    mia.stages().stream().skip(1).forEach(stage -> {
      fullMap.put(stage, new RegisterTensor(control.identifier.append(stage.simpleName() + "_full"),
          Collections.singletonList(new RegisterTensor.Dimension(0, Type.bool(), 1))));
    });
    for (Stage stage : mia.stages()) {
