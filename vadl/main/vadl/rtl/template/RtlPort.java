...
public record RtlPort(
  ...
  public String getType() {
    if (resource instanceof Signal sig) {
      return RtlType.of(resource.resultType());
    }
    if (resource instanceof RegisterTensor reg) {
      if (reg.hasAddress()) {
        ...
        if (addrType != null) {
          if (read) {
            return "new VADL.RegFileReadPort(%s, %s)"
                .formatted(RtlType.of(reg.resultType()), addrType.bitWidth());
          } else {
        ...
        if (read) {
          return "new VADL.RegReadPort(%s)".formatted(RtlType.of(reg.resultType()));
        } else {
          return "new VADL.RegWritePort(%s)".formatted(RtlType.of(reg.resultType()));
        }
      }
    }
    if (resource instanceof Memory mem) {
      if (read) {
        return "new VADL.MemReadPort(%s, %s)"
            .formatted(RtlType.of(mem.resultType()), mem.addressType().bitWidth());
      } else {
