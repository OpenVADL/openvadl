...
package vadl.vdt.passes.validate;
...
/**
 * Validates the encoding constraints of a given encoding.
 */
public class EncodingConstraintValidator {

  private final Encoding encoding;

  /**
   * Creates a new validator for the given encoding.
   *
   * @param encoding The encoding to validate.
   */
  public EncodingConstraintValidator(Encoding encoding) {
    this.encoding = encoding;
  }

  /**
   * Validates the encoding constraints of the encoding.
   *
   * <p>This method checks that the encoding constraints are well-formed and do not contain
   * any invalid operations, such as resource reads or field accesses.</p>
   */
  public void check() {
   ...
   *   <li>If p and q are formulas, then {@code p && q} is a formula.</li>
  ...
  private void checkFormular(ExpressionNode formular) {
    ...
    if (builtIn == BuiltInTable.AND || builtIn == BuiltInTable.OR) {
