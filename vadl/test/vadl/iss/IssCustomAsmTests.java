...
abstract public class IssCustomAsmTests extends QemuIssTest {

  protected abstract Path asmTestDir();

  protected abstract Path specPath();

  protected Stream<DynamicTest> runTests() {
    var qemuImage = generateIssSimulator(specPath().toString());

    // Find test source directory and results directory
    var testSources = getTestSourcePath(asmTestDir().toString());
    ...
    return testSourceFiles.stream()
        .map(test -> DynamicTest.dynamicTest(test, () -> {
          ...
          var failed = !resultOutput.contains("SUCCESS");
          StringBuilder sb = new StringBuilder();

          // Parse the result file
          if (failed) {
            sb.append("Test failed: " + test + "\n");
          }
